...
; compare string 1 and 2.
	...
	mov	rdi,fmt1
	cmp	rax,0
	je	eql1		; strings are equal.
	mov	rdi,fmt11	; strings are unequal.
eql1:
	mov	rsi,rax
	xor	rax,rax
	call	printf
; compare string 2 and 3.
	...
	mov	rdi,fmt22	; strings are unequal.
eql2:
	mov	rsi,rax
	xor	rax,rax
	call	printf
...
.loop:
	...
	jmp	.loop
.differ:
	mov	rax,rbx	;rax contains the position of the differing character
	add	rax,rcx ;because the index starts at 0
	inc	rax
.equal:
leave
ret
