...
; create mode (permissions).
	S_IRUSR		 equ	00400q		; user read permissions.
	S_IWUSR		 equ	00200q		; user write permissions.
	NL		 equ	0xa		; End of line.
	bufferlen	 equ	64
	fileName	 db	"testfile.txt",0
	FD		 dq	0		; File descriptor.

	text1		 db	"1. Hello...to everyone!",NL,0
	len1		 dq	$-text1-1	; remove NL.
	text2		 db	"2. Here I am!",NL,0
	len2		 dq	$-text2-1	; remove NL.
	text3		 db	"3. Alife and Kicking!",NL,0
	len3		 dq	$-text3-1	; remove NL.
	text4		 db	"Adios !!!",NL,0
	len4		 dq	$-text4-1	; remove NL.
...
readFile:
	mov	rax, NR_read
	syscall 			; rax - number of characters to read.
	cmp	rax, 0
	jl	readerror
	mov	byte [rsi+rax], 0	; Add NL.
	mov	rax, rsi
...
createFile:
	...
	syscall
	cmp	rax, 0				; File desriptor in rax.
	jl	createerror
